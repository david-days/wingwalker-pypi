...
try:
    ...
    print(f'Directories {vtk_dir} already exists')
...
@pytest.mark.threeD
@pytest.mark.parametrize('wing_side', [WingType.LEFT, WingType.RIGHT])
def test_elliptical_wing_vtk(wing_side: WingType):
    """
    Takes a standard model generation and attempts to create a PyVista point cloud out of it
    Args:
        wing_side:
    ...
    f_name = os.path.join(vtk_dir, f'elliptical_wing_{wing_side.name}.vtk')
    if os.path.exists(f_name):
        os.remove(f_name)
    export_vtk(model, f_name)
    assert os.path.exists(f_name)


@pytest.mark.threeD
@pytest.mark.parametrize('wing_side', [WingType.LEFT, WingType.RIGHT])
def test_rectangular_wing_vtk(wing_side: WingType):
    """
    Takes a standard model generation and attempts to create a PyVista point cloud out of it
    Args:
        wing_side:
    ...
    f_name = os.path.join(vtk_dir, f'rectangular_wing_{wing_side.name}.vtk')
    if os.path.exists(f_name):
        os.remove(f_name)
...
@pytest.mark.threeD
@pytest.mark.parametrize('wing_side', [WingType.LEFT, WingType.RIGHT])
def test_geometric_wing_vtk(wing_side: WingType):
    """
    Takes a standard model generation and attempts to create a PyVista point cloud out of it
    Args:
        wing_side:
    ...
    f_name = os.path.join(vtk_dir, f'geometric_wing_{wing_side.name}.vtk')
    if os.path.exists(f_name):
        os.remove(f_name)
