...
class WingSection(FullWing):
    ...
    def __init__(self, airfoil_sections: list[AirfoilSection], wing_type: WingType = WingType.UNDEFINED, planform: Planform = Planform.UNDEFINED):
        ...
        self. end_t: float = -1.0

    def __str__(self)->str:
        return f'Wing Section: {self.wing_type.name} at {self.start_t} -> {self.end_t}'

    def __repr__(self)->str:
        return f'WingSection(airfoil_sections=[], wing_type={self.wing_type.name}, planform={self.planform.name})'